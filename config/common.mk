...
# Backup Tool
PRODUCT_COPY_FILES += \
    vendor/sprit/prebuilt/common/bin/backuptool.sh:install/bin/backuptool.sh \
    vendor/sprit/prebuilt/common/bin/backuptool.functions:install/bin/backuptool.functions \
    vendor/sprit/prebuilt/common/bin/50-lineage.sh:$(TARGET_COPY_OUT_SYSTEM)/addon.d/50-lineage.sh

ifneq ($(AB_OTA_PARTITIONS),)
PRODUCT_COPY_FILES += \
    vendor/sprit/prebuilt/common/bin/backuptool_ab.sh:$(TARGET_COPY_OUT_SYSTEM)/bin/backuptool_ab.sh \
    vendor/sprit/prebuilt/common/bin/backuptool_ab.functions:$(TARGET_COPY_OUT_SYSTEM)/bin/backuptool_ab.functions \
    vendor/sprit/prebuilt/common/bin/backuptool_postinstall.sh:$(TARGET_COPY_OUT_SYSTEM)/bin/backuptool_postinstall.sh
...
PRODUCT_COPY_FILES += \
    vendor/sprit/config/permissions/backup.xml:$(TARGET_COPY_OUT_SYSTEM)/etc/sysconfig/backup.xml

# Lineage-specific broadcast actions whitelist
PRODUCT_COPY_FILES += \
    vendor/sprit/config/permissions/lineage-sysconfig.xml:$(TARGET_COPY_OUT_SYSTEM)/etc/sysconfig/lineage-sysconfig.xml

# Copy all Lineage-specific init rc files
$(foreach f,$(wildcard vendor/sprit/prebuilt/common/etc/init/*.rc),\
	$(eval PRODUCT_COPY_FILES += $(f):$(TARGET_COPY_OUT_SYSTEM)/etc/init/$(notdir $f)))

# Copy over added mimetype supported in libcore.net.MimeUtils
PRODUCT_COPY_FILES += \
    vendor/sprit/prebuilt/common/lib/content-types.properties:$(TARGET_COPY_OUT_SYSTEM)/lib/content-types.properties

# Enable Android Beam on all targets
PRODUCT_COPY_FILES += \
    vendor/sprit/config/permissions/android.software.nfc.beam.xml:$(TARGET_COPY_OUT_SYSTEM)/etc/permissions/android.software.nfc.beam.xml
...
# This is Lineage!
PRODUCT_COPY_FILES += \
    vendor/sprit/config/permissions/org.lineageos.android.xml:$(TARGET_COPY_OUT_SYSTEM)/etc/permissions/org.lineageos.android.xml
...
include vendor/sprit/config/aosp_audio.mk

# Include Lineage audio files
include vendor/sprit/config/lineage_audio.mk

ifneq ($(TARGET_DISABLE_LINEAGE_SDK), true)
# Lineage SDK
include vendor/sprit/config/lineage_sdk_common.mk
endif

# TWRP
ifeq ($(WITH_TWRP),true)
include vendor/sprit/config/twrp.mk
endif
...
PRODUCT_ENFORCE_RRO_EXCLUDED_OVERLAYS += vendor/sprit/overlay
DEVICE_PACKAGE_OVERLAYS += vendor/sprit/overlay/common
...
PRODUCT_EXTRA_RECOVERY_KEYS += \
    vendor/sprit/build/target/product/security/lineage
...
-include vendor/sprit/config/partner_gms.mk
