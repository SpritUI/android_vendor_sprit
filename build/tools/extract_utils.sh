...
function setup_vendor() {
    ...
    export SPRIT_ROOT="$3"
    if [ ! -d "$SPRIT_ROOT" ]; then
        echo "\$SPRIT_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SPRIT_ROOT/$OUTDIR" ]; then
        mkdir -p "$SPRIT_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$SPRIT_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$SPRIT_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$SPRIT_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SPRIT_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR The SpritUI Project\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR The SpritUI Project\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR The SpritUI Project\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The SpritUI Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The SpritUI Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The SpritUI Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The SpritUI Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The SpritUI Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SPRIT_ROOT"/prebuilts/tools-lineage/common/smali/baksmali.jar
        export SMALIJAR="$SPRIT_ROOT"/prebuilts/tools-lineage/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$SPRIT_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$SPRIT_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$SPRIT_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$SPRIT_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$SPRIT_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$SPRIT_ROOT"/vendor/sprit/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SPRIT_ROOT"/"$OUTDIR"/radio
