function __print_sprit_functions_help() {
cat <<EOF
Additional SpritUI functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- spritgerrit:     A Git wrapper that fetches/pushes patch from/to SpritUI Gerrit Review. //IT'S NOT WORKING NOW
- spritrebase:     Rebase a Gerrit change and push it again. //IT'S NOT WORKING NOW
- spritremote:     Add git remote for SpritUI Gerrit Review. //IT'S NOT WORKING NOW
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for SpritUI Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Sprit model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch sprit_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/sprit-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.sprit.device | grep -q "$SPRIT_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $SPRIT_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function spritremote()
{
    ...
    git remote rm sprit 2> /dev/null
    ...
    local SPRIT_USER=$(git config --get review.review.spritos.org.username)
    if [ -z "$SPRIT_USER" ]
    then
        git remote add sprit ssh://review.spritos.org:29418/$PFX$PROJECT
    else
        git remote add sprit ssh://$SPRIT_USER@review.spritos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'sprit' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/SpritUI/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.sprit.device | grep -q "$SPRIT_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        spritremote
        git push sprit HEAD:refs/heads/'$1'
    fi
    '
}

function spritgerrit() {
    ...
    local user=`git config --get review.review.spritos.org.username`
...
function spritrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "SpritUI Gerrit Rebase Usage: "
        echo "      spritrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.spritos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.sprit.device | grep -q "$SPRIT_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/sprit/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $SPRIT_FIXUP_COMMON_OUT ]; then
